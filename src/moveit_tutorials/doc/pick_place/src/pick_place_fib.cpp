...
void hoverPose(moveit::planning_interface::MoveGroupInterface& move_group, float x, float y, ros::Publisher goal_pub)
{ 
  // We can plan a motion for this group to a desired pose for the
  // end-effector.
...
void pickPose(moveit::planning_interface::MoveGroupInterface& move_group_interface, std::string direction, ros::Publisher goal_pub){
  std_msgs::Bool status;
...
int main(int argc, char** argv)
{
  ...
  { 
    ...
    // grab_pose.position.x = pick_x[i];
    // grab_pose.position.y = pick_y[i];
    ...
    ROS_WARN("--------------------");
    pickPose(group_arm , "down", goal_pub);
    ...
    pickPose(group_arm , "up", goal_pub);
